...
class FullBody extends StatefulWidget {
  final Map dObj;
  const FullBody({super.key, required this.dObj});
...
class _FullBodyState extends State<FullBody> {
  ...
  late DatabaseReference _workoutsRef;
  ...
  void initState() {
    super.initState();
    _controllersMap = {};
    _workoutsRef = FirebaseDatabase.instance.reference().child('users').child(FirebaseAuth.instance.currentUser!.uid).child('workouts');
    // Initialize YoutubePlayerControllers and other setup...
